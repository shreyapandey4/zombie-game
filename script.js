 ...
 function spawnZombie() {
  const safeZoneRadius = Math.min(canvas.width, canvas.height) / 4; // Dynamic safe zone radius

  let x, y;

  // Continue generating spawn coordinates until they fall outside the safe zone
  do {
    const side = Math.floor(Math.random() * 4); // 0 = top, 1 = right, 2 = bottom, 3 = left
    if (side === 0) {
      x = Math.random() * canvas.width;
      y = -SPRITE_SIZE; // Spawn above the canvas
    } else if (side === 1) {
      x = canvas.width + SPRITE_SIZE; // Spawn to the right of the canvas
      y = Math.random() * canvas.height;
    } else if (side === 2) {
      x = Math.random() * canvas.width;
      y = canvas.height + SPRITE_SIZE; // Spawn below the canvas
    } else {
      x = -SPRITE_SIZE; // Spawn to the left of the canvas
      y = Math.random() * canvas.height;
    }
  } while (Math.hypot(x - (character.x + character.width / 2), y - (character.y + character.height / 2)) < safeZoneRadius);
